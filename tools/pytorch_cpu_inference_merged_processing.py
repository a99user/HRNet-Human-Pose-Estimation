...
def main():
    ...
    model = eval('models.'+cfg.MODEL.NAME+'.get_pose_net_with_processing')(
        cfg, is_train=False
    )
    ...
    if args.convert_onnx:
        ...
        torch.onnx.export(model.cpu(), x_tensor.cpu(), 'model_with_processing.onnx', export_params=True,
    ...
    with torch.no_grad():
        for i, (input, target, target_weight, meta) in enumerate(valid_loader):
            ...
            save_debug_images(cfg, input, meta, target, coords * 4, batch_heatmaps,
                              prefix)

            if i == 100:
                break

        logger.info(f'PyTorch: Average processing time of model with merged pre- and post-processing:{process_time.avg}')
